...
const TableMesin = () => {
    ...
    const [currentSelected, setCurrentSelected] = useState<PayloadDeviceType>({ id_mesin: "", nama_dinas: "", nama_stasiun: "", id: 0 })
    const { data, isLoading } = useQuery({ queryKey: ["device", offset], queryFn: ()=> getPaginateDevice({ limit: 10, offset, token: token || ''})})
    ...
    return (
          ...
          <table className="mt-4 w-full min-w-max table-auto text-left">
            ...
            <tbody>
              {data?.data?.values && data?.data?.values.map(
                ({ id_mesin, nama_dinas, nama_stasiun, id }: PayloadDeviceType, index: number) => {
                      ...
                      <td className={classes}>
                        <Tooltip content="Edit User">
                          <IconButton variant="text" onClick={()=>{
                            setCurrentSelected({ id, nama_dinas, id_mesin, nama_stasiun })
      ...
      <DeleteMesinDialog isOpen={isOpenDeleteDialog} data={currentSelected} handleOpen={setIsOpenDeleteDialog}/>
      <AddMesinDialog isOpen={isOpenAddDialog} handleOpen={setIsOpenAddDialog}/>
      <EditMesinDialog isOpen={isOpenEditDialog} handleOpen={setIsOpenEditDialog} defaultData={currentSelected}/>
      </>
    );
  };
