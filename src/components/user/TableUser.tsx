...
const TABLE_HEAD = ["#", "username", "Nama Instansi", "API Key", "Screet Key", "", ""];
...
const TableUser = () => {
  ...
  const [currentSelected, setCurrentSelected] = useState<TableUserType>({ username: "", nama_dinas: "", api_key: "", secret_key: "", id: 0 })
  const { data, isLoading } = useQuery({ queryKey: ["user", offset], queryFn: ()=> getPaginateUser({ limit: 10, offset, token: token || ''})})
  ...
  return (
    ...
    <Card className="h-full w-full">
      ...
      <CardBody className="overflow-x-auto px-0 !-mt-8">
        <table className="mt-4 w-full min-w-max table-auto text-left">
          ...
          <tbody>
            {data?.data?.values && data?.data?.values.map(
              ({ username, nama_dinas, api_key, secret_key, id }: TableUserType, index: number) => {
                    ...
                    <td className={classes}>
                      <Tooltip content="Edit User">
                        <IconButton variant="text" onClick={()=>{
                          setCurrentSelected({ id, nama_dinas, api_key, secret_key, username })
    ...
    <DeleteUserDialog isOpen={isOpenDeleteDialog} data={currentSelected} handleOpen={setIsOpenDeleteDialog}/>
    <AddUserDialog isOpen={isOpenAddDialog} handleOpen={setIsOpenAddDialog}/>
    <EditUserDialog isOpen={isOpenEditDialog} handleOpen={setIsOpenEditDialog} defaultData={currentSelected}/>
    </>
  );
};
